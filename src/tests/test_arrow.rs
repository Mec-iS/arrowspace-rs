...
fn test_builder_unit_norm_items_invariance_under_normalisation_toggle() {
    ...
    println!("Generated {} unit-normalized items with {} features", items.len(), items[0].len());
    ...
    assert_relative_eq!(
        stats_norm.mean_degree, stats_raw.mean_degree, epsilon = 2e-1
    );
    ...
    println!("Raw lambdas (first 5): {:?}", &lambdas_raw[..5.min(lambdas_raw.len())]);

    assert_eq!(
        lambdas_norm.len(), lambdas_raw.len(),
    ...
    for (i, (lam_norm, lam_raw)) in lambdas_norm.iter().zip(lambdas_raw.iter()).enumerate() {
        ...
        if diff > 1e-10 {
            println!("Lambda {} difference: norm={:.12}, raw={:.12}, diff={:.2e}", 
...
fn test_builder_direction_vs_magnitude_sensitivity() {
    ...
    println!("Normalized lambdas (first 3): {:?}", &lambdas_norm[..3.min(lambdas_norm.len())]);
    println!("Tau lambdas (first 3): {:?}", &lambdas_tau[..3.min(lambdas_tau.len())]);
}
...
fn test_builder_normalisation_flag_is_preserved() {
    ...
    assert_eq!(gl.graph_params.normalise, false, "normalise flag must be preserved");
}
...
fn test_builder_clustering_produces_valid_assignments() {
    ...
    let assigned_count = aspace.cluster_assignments.iter().filter(|x| x.is_some()).count();
    assert!(
        assigned_count > 0,
...
fn test_builder_spectral_laplacian_computation() {
    ...
    println!("No spectral - signals shape: {:?}", aspace_no_spectral.signals.shape());
    println!("With spectral - signals shape: {:?}", aspace_spectral.signals.shape());
...
fn test_builder_with_normalized_vs_unnormalized_items() {
    ...
    println!("Normalized lambdas: {:?}", &lambdas_norm[..3.min(lambdas_norm.len())]);
    println!("Unnormalized lambdas: {:?}", &lambdas_unnorm[..3.min(lambdas_unnorm.len())]);
    ...
    println!("Unnormalized diagonals: {:?}", &d_unnorm[..3.min(d_unnorm.len())]);
...
fn test_builder_lambda_statistics() {
    ...
    assert!(min >= 0.0, "All lambdas should be non-negative, got min={}", min);
...
fn test_builder_cluster_radius_impact() {
    ...
    assert!(aspace.cluster_radius > 0.0, "Cluster radius should be positive");
}
