...
use arrow::{array::{Float64Array, RecordBatch, StringArray, UInt64Array}, datatypes::{DataType, Field, Schema}};
...
use crate::{builder::{ArrowSpaceBuilder, ConfigValue}, storage::StorageError};
...
pub fn load_sparse_matrix(path: impl AsRef<Path>) -> Result<CsMat<f64>, StorageError> {
    ...
    // Extract triplets
    let row_col = batch.column_by_name("row")
        .and_then(|c| c.as_any().downcast_ref::<UInt64Array>())
        .ok_or_else(|| StorageError::Invalid("row missing".to_string()))?;
    
    let col_col = batch.column_by_name("col")
        .and_then(|c| c.as_any().downcast_ref::<UInt64Array>())
        .ok_or_else(|| StorageError::Invalid("col missing".to_string()))?;
    
    let val_col = batch.column_by_name("value")
        .and_then(|c| c.as_any().downcast_ref::<Float64Array>())
...
pub fn save_arrowspace_checkpoint_with_builder(
    ...
    save_dense_matrix(raw_data, base_path, &format!("{}_raw_data", checkpoint_name), None)?;
    save_sparse_matrix(adjacency, base_path, &format!("{}_adjacency", checkpoint_name), None)?;
    save_dense_matrix(centroids, base_path, &format!("{}_centroids", checkpoint_name), None)?;
    save_sparse_matrix(laplacian, base_path, &format!("{}_laplacian", checkpoint_name), None)?;
    save_sparse_matrix(signals, base_path, &format!("{}_signals", checkpoint_name), None)?;
    ...
    let artifacts = vec![
        ...
        ("adjacency", "sparse", adjacency.shape(), Some(adjacency.nnz())),
        ("centroids", "dense", centroids.shape(), None),
        ("laplacian", "sparse", laplacian.shape(), Some(laplacian.nnz())),
